required_packages <- c(
  ...
  "dplyr","ggplot2","mirt","psych","GPArotation",
  ...
  dashboardBody(
    ...
    tabItems(
        ...
        fluidRow(
          box(title="📁 Upload Data", status="primary", solidHeader=TRUE, width=6,
            tags$div(class="info-callout",
              tags$b("Format:")," CSV atau XLSX. Baris=examinee, Kolom=item.",tags$br(),
      ...
      tabItem(tabName="assumptions",
        ...
        fluidRow(
          box(title="🔍 Analisis Faktor Eksploratori", status="info", solidHeader=TRUE, width=4,
            fluidRow(
              column(6,numericInput("n_factors","Faktor:",value=1,min=1,max=10)),
              column(6,selectInput("fa_method","Rotasi:",
                choices=c("none","varimax","oblimin","promax")))
            ),
            actionButton("run_fa","▶ Analisis Faktor",class="btn-primary"),
            hr(),
            withSpinner(verbatimTextOutput("fa_result"),type=4)
          ),
      ...
      tabItem(tabName="irt_model",
        fluidRow(
          box(title="⚙️ Konfigurasi Model", status="primary", solidHeader=TRUE, width=4,
            tags$div(class="info-callout",
              tags$b("Chapter 3:")," Estimasi parameter item & ability."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_fa, {
    ...
    if (input$use_msi && rv$data_type=="polytomous") df <- msi_transform(df)
    tryCatch({
      far <- fa(df,nfactors=input$n_factors,
        rotate=if(input$fa_method=="none")"none"else input$fa_method,fm="ml")
      output$fa_result <- renderPrint({
        cat("=== Factor Loadings (cutoff=0.3) ===\n")
        print(far$loadings,cutoff=0.3)
        cat("\n=== Variance Explained ===\n")
        print(round(far$Vaccounted,3))
      })
    }, error=function(e) output$fa_result <- renderPrint(cat("Error:",e$message)))
  })
